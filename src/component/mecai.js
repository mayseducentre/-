...
export default function MecAiProGroq() {
  ...
  const GROQ_KEY = process.env.REACT_APP_GROQ;
  ...
  useEffect(() => {
    if (messages.length === 0) {
      typewriter(
        "👋 Welcome to MEC AI — your smart school assistant! I can explain computing concepts, solve simple problems, or even compliment your uploaded pictures. Try asking: 'What is HTML?' or 'Solve 12 + 5'."
      );
    }
  }, []);
  ...
  const nowTime = (ts) => new Date(ts).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  ...
  async function callGroq(prompt) {
    try {
      ...
      return data?.choices?.[0]?.message?.content || pick(FALLBACKS);
    } catch {
      ...
      <div style={styles.container}>
        <header style={styles.header}>
          <div>
            ...
            <div style={styles.subtitle}>Your School & Tech Assistant</div>
          </div>
